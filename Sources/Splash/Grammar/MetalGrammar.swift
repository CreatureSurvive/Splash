...
/// Grammar for the Metal Shading Language
public struct MetalGrammar: Grammar {
...
private extension MetalGrammar {
    static let keywords = ([
        "alignas", "alignof", "and", "and_eq", "asm", "auto", "bitand", "bitor", "bool", "break", "case",
        "catch", "char", "char8_t", "char16_t", "char32_t", "class", "compl", "concept", "const", "constant",
        "consteval", "constexpr", "constinit", "const_cast", "continue", "decltype", "default", "delete",
        "device", "do", "double", "dynamic_cast", "else", "enum", "explicit", "export", "extern", "false",
        ...
        "static_assert", "static_cast", "struct", "switch", "template", "this", "thread", "throw", "true",
        "try", "typedef", "typeid", "typename", "union", "unsigned", "using", "void", "volatile", "wchar_t",
        "while", "xor", "xor_eq"
    ] as Set<String>).union(accessControlKeywords)

    static let commonMetalTypes: Set<String> = ([
        "bool2", "bool4", "char3", "uchar3", "short2", "ushort2", "short4", "ushort4", "int3", "uint3",
        "bool3", "char2", "uchar2", "char4", "uchar4", "short3", "ushort3", "int2", "uint2", "int4",
        "uint4", "long2", "ulong2", "long3", "ulong3", "long4", "ulong4", "half2", "half3", "half4",
        "bfloat2", "bfloat3", "bfloat4", "float2", "float3", "float4"
    ] as Set<String>)
    ...
    struct PreprocessingRule: SyntaxRule {
        ...
        private let directiveTokens: Set<String> = ["#include", "#warning", "#error"]
    ...
    struct NumberRule: SyntaxRule {
        ...
        private let numericLiteralSuffixes: Set<String> = (["h", "f"] as Set<String>)

        private func stripLiteralSuffix(_ token: String) -> String {
            var stripped = token
            for suffix in numericLiteralSuffixes {
                if stripped.hasSuffix(suffix) {
                    stripped = stripped.removing(suffix)
                }
            }
            return stripped
        }
        ...
        func matches(_ segment: Segment) -> Bool {
            // Ordinary integers (TODO: handle numeric literal suffixes for integers)
            ...
            if let previous = segment.tokens.previous {
                if previous == "." {
                    return stripLiteralSuffix(segment.tokens.current).isNumber
                }
            }
            ...
            let nextWithoutSuffix = stripLiteralSuffix(next)
            return previous.isNumber && nextWithoutSuffix.isNumber
        }
    }
    ...
    struct CallRule: SyntaxRule {
        ...
        func matches(_ segment: Segment) -> Bool {
            ...
            if let previousToken = segment.tokens.previous {
                ...
                // Don't treat parameterized attributes as function calls
                if previousToken == "[[" {
                    return false
                }
    ...
    struct TypeRule: SyntaxRule {
        ...
        func matches(_ segment: Segment) -> Bool {
            ...
            guard token.isCapitalized else {
                return commonMetalTypes.contains(token)
            }
